...
from reachy_assistant.tts.live_tts import speak_live
...
def main():
    ...
    mini = None
    if ReachyMini is not None:
     try:
        mini = ReachyMini(media_backend="webrtc")  # prøv Reachy
        print("Reachy Mini: tilkoblet (TTS via robot).")
     except Exception as e:
        print(f"Reachy Mini: ikke tilgjengelig, bruker PC-lyd. ({e})")
        mini = None
    else:
     print("reachy_mini ikke installert, bruker PC-lyd.")
    ...
    while True:
        ...
        if q.lower().startswith("/say "):
            text = q[5:].strip()
            if text:
                speak_live(text, voice=TTS_VOICE, mini=mini)  # mini=None -> PC-lyd
            continue
        ...
        if tts_enabled:
            try:
                speak_live(answer, voice=TTS_VOICE, mini=mini)
            except Exception as e:
                print(f"(TTS-feil, fortsetter uten lyd): {e}")
